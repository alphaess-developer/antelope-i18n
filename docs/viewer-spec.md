...
| 节 | 内容 |
|---|---|
| **三类任务**（三张并排卡片） | ① 走查并修正已有译文（定位 ns → 单 ns 导出 → Excel 走查 → 改 JSON）② 新增文案（只写 `en-US`，fill-missing 补齐）③ 新增/修正错误码（数值排序、位置参数不能改） |
| **写回仓库** | 共同步骤：网页编辑 / github.dev 两种方式 + `提 PR → CI → 合并 → 重新构建 → 生效` 流程条 |
