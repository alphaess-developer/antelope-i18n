...
> ⚠️ 「只写 en-US + 英文占位后续优化」作为新增默认路径**已停用**——PR 应直接带全量真译文。
...
① 新增文案（开发/PM）
   ...
   （推荐用 AI / prompts/ 一次写齐；以 en-US 为源）

② 修正已有译文 / 消化历史英文占位（PM/AI）
   直接改对应语种文件；存量英文占位可用 prompts/translate.md 批量替换
```

**关键：`en-US` 必须有，且 PR 须带齐 11 语种真译文。** `en-US` 是源，缺了它这条 key 就没有源文（CI 报 `extraKeys`）。

> 可先跑 `node tools/fill-missing.mjs --write` 打好格式正确的骨架，再立刻换成真译文 ——
> 路径、缩进、key 顺序由脚本保证。**不要把英文占位当作可合并的终态。**
> 现有 ns 的 11 个语种文件都已齐全，所以**不需要新建文件**。
