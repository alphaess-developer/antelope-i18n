...
> **状态（2026-08-06）：draft / 英文占位待优化流程暂时停用。**  
> 当前期望：PR 直接提交 11 语种真译文。`fill-missing.mjs` 脚本仍保留，供本地检查缺 key / 建骨架；不要把英文占位当作可合并的最终内容。

历史决策：后端/PM 只加基准语言，其余 10 个语种由 workflow 自动用英文占位补齐，并在 `_meta` 标 draft。

**为什么用英文占位而非留空**：宿主项目配置 `fallbackLng: false`，缺 key 会**直接把 key 名显示给用户**。显示英文远好于显示 `some_key_name`。

**为什么不在 CI 调 AI**：不需要配密钥；英文占位是可接受的降级，与「产品持续优化」的工作流一致（该工作流现已暂停）。
...
且 draft / 英文占位待优化流程已暂时停用（见 D8），短期内不会有稳定的 draft 数据源。数据契约里已留好位置，若日后恢复 draft 流程再补列。
...
3. **自动补齐语种** —— 历史设计用 fill-missing 消摩擦；**2026-08 起暂时改为提交全量真译文**（见 D8）
