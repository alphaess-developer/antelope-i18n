...
### D17 · AI 协作：AI 改文件并自跑校验，git 归人

工作流见 [ai-workflow.md](ai-workflow.md)，任务提示词在 [`prompts/`](../prompts/)。

```
人   定范围  →  AI  改 locales/*.json + 跑 validate 自查  →  人  看 diff、commit、push、开 PR
```

**AI 不给 git 写权限**（`commit` / `push` / `gh pr create`）。评估过的三档：

| 档 | 取舍 |
|---|---|
| 只生成内容，人工落地 | 最安全，但 AI 跑不了 `validate`，机械错误全靠 CI 往返 —— 浪费掉零依赖脚本的最大价值 |
| **改文件 + 自跑校验，人工提交** ← 选这个 | AI 能自证合规；PR 作者仍是人，审计线索完整 |
| 全自动到 PR | 效率最高，但把写权限交给 AI，且 CODEOWNERS 目前全是占位用户名（§4 待办 #1），review 分派本来就不生效 |

**分界线的依据**：`tools/*.mjs` 零依赖、几秒跑完（D12），所以「改完自证合规」这段交给 AI 边际收益最大；
而 `main` 有分支保护、PR 要人答辩，git 历史又是半年后追查「这条德语谁改的」的唯一线索 —— 作者必须是人。
