...
> **状态（2026-08-06）：文档约定已改为「PR 直接提交 11 语种真译文」。**  
> `fill-missing` 脚本 / workflow 仍保留，作本地建骨架或 CI 兜底（防 key 对齐变红），
> **不再**作为新增文案的推荐终态。原先「只写英文 + 产品后续优化」的路径停用。

历史决策：后端/PM 只加基准语言，其余 10 个语种由 workflow 自动用英文占位补齐。

> 原先还会在 `_meta/` 标 draft，**这部分已移除**，见 D15。

**为什么用英文占位而非留空**：宿主项目配置 `fallbackLng: false`，缺 key 会**直接把 key 名显示给用户**。显示英文远好于显示 `some_key_name`。

**为什么不在 CI 调 AI**：不需要配密钥；英文占位曾是可接受的降级（现改为要求提交真译文）。
...
### D17 · AI 协作：AI 一路做到 PR，人审 PR、人合并

> **本条已修订（2026-08-07）：从「git 归人」放开到「AI 可开 PR」。** 原决策与修订理由都在下面。

工作流见 [ai-workflow.md](ai-workflow.md)，任务提示词在 [`prompts/`](../prompts/)。

```
人  定范围  →  AI  改 locales/*.json + validate 自查 + 建分支 commit push 开 PR  →  人  审 PR、点合并
```
...
| 档 | 取舍 |
|---|---|
| 只生成内容，人工落地 | 最安全，但 AI 跑不了 `validate`，机械错误全靠 CI 往返 —— 浪费掉零依赖脚本的最大价值 |
| 改文件 + 自跑校验，人工提交 | ~~2026-08-06 选这个~~ 见下方修订 |
| **全自动到 PR** ← 现在选这个 | 委托人只需给内容和审 PR |

**边界线划在「开 PR」和「合并」之间，不是「改文件」和「commit」之间：**

| AI 可以 | AI 不可以 |
|---|---|
| 建分支、commit、push、`gh pr create`、CI 红了追加 commit | **`gh pr merge`** —— 任何情况都不行 |
| | 直推 `main`、`push --force`、改分支保护、往 baseline 加豁免 |

#### 为什么从「git 归人」改成「AI 可开 PR」

原决策的三条理由，逐条重新审视：

| 原理由 | 现在怎么看 |
|---|---|
| 「PR 是人的责任，谁提谁答辩」 | **混淆了「提 PR」和「落地」。** 开 PR 不是落地，是**提交待审** —— 后面还有 CI 阻塞级检查和人工点合并两道关。让 AI 走到待审状态，不降低任何安全边界 |
| 「CODEOWNERS 会 request review」 | 当时说「CODEOWNERS 全是占位用户名，review 分派本来就不生效」——**这条已过期**，`.github/CODEOWNERS` 现在填的是真实用户名 |
| 「git 历史是审计线索，作者必须是人」 | 责任人不等于 commit author。PR 描述写明「谁委托、AI 生成」+ commit 的 `Co-Authored-By` 同样可追溯，答案从「某人改的」变成「X 委托 AI 在 PR #N 改的」，精度没降 |

**真正促成修改的观察**：后端同事为了加一条错误码，要学 branch / commit / push —— 这是整条链路上
最大的摩擦，而它卡在**最机械、最不需要判断力**的一步。AI 已经把 11 个语种都改好、校验也跑过了，
让人接手做剩下三条命令，收益只有「commit author 是人名」这一项，代价是很多人干脆不改了。

#### 放开后新增的约束

| 约束 | 落点 |
|---|---|
| PR 描述必须写明「AI 生成 + 委托人」 | `ai-workflow.md` §5.1 |
| commit 用 `Co-Authored-By:` 标模型 | 同上 |
| 动手前必须 `git fetch` 确认不是基于过时快照 | `ai-workflow.md` §5.2 |
| review 清单加「范围失控」「baseline 是否被动过」 | `ai-workflow.md` §7 |

> ⚠️ **已知弱点：CODEOWNERS 与 PR 作者同账号时，GitHub 不会请求 review。** 目前 AI 用的账号
> 就是唯一的 code owner，所以「有人审」靠委托人自觉而非机制。多人协作后应给 AI 单独账号或 bot token
> —— 记在 §4 待办。

> **§5.2 那条不是凑数的**：本次修订前有一个 PR 就是基于落后 16 个提交的本地 main 写的，
> 重复实现了已有的帮助 Tab，还把已移除的 `_meta` draft 机制（D15）写回文档，只能作废重来。

#### 规则只写一份，角色入口很薄

`ai-workflow.md` §4 是**唯一**的规则表述，按任务组织；后端/产品各有约十行入口，只说
「你的范围是哪些文件、用哪个提示词」。

**为什么不给两个角色各写一份完整文档**：占位符铁律、校验流程、交付格式对两者完全相同，
写两份必然漂移 —— 改一处漏一处，然后两份互相矛盾。这与 `viewer-spec.md` §4.7
帮助 Tab「按功能不按身份」是同一条理由。

> `prompts/` 里的三份提示词**刻意内联了铁律的压缩版** —— 这是有意的重复，因为它们要能被整份粘贴给
> 读不到仓库的 AI（ChatGPT 网页版）。每份都注明「若能读仓库，`CLAUDE.md` 优先」，
> 且这些规则本身有 CI 阻塞级检查兜底，漂移会被挡住。
...
3. **自动补齐语种** —— 历史设计用 fill-missing 消摩擦；**2026-08 起文档改为要求提交全量真译文**（见 D8）
