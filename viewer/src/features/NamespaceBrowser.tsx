 ...
 *
 * 目前只有字典 Tab 一个实例 —— 产品配置 Tab 曾是第二个，动态表单文案迁走后它只剩
 * 「改了不生效的旧副本 + 开发维护的页面文案」，作为 PM 工作面反而是干扰，已移除。
 * 保留本组件是因为「一组同前缀 ns」这个形态还会再出现（如 device-form/*），
 * 到时只需再写一个薄封装。**单个 ns 不要用它** —— 左侧列表只有一项没有意义，照错误码 Tab 写。
 *
